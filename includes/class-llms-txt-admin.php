...
class LLMS_Txt_Admin
{
     ...
     *
     * @since 1.0.0
     * @var LLMS_Txt_Admin|null
     */
    private static ?LLMS_Txt_Admin $instance = null;
     ...
     */
    public static function get_instance(): LLMS_Txt_Admin
    {
        ...
        if (isset($input['post_types']) && is_array($input['post_types'])) {
            ...
            if (isset($input['cpt_content_source']) && is_array($input['cpt_content_source'])) {
                ...
                foreach ($sanitized['post_types'] as $post_type) {
                    ...
                    if (
                        isset($sanitized['cpt_content_source'][$post_type]) &&
    ...
    public function enqueue_scripts(string $hook): void
    {
        ...
        $is_plugin_page = (
            'settings_page_llms-txt-generator' === $hook ||
            strpos($hook, 'llms-txt') !== false ||
            strpos($current_page, 'llms-txt') !== false ||
            $current_page === 'llms-txt-generator'
        );

        // Não carregar assets em outras páginas
        if (!$is_plugin_page) {
            return;
        }
    ...
    public function field_site_description_callback()
    {
        ...
        ?>
        <textarea name="llms_txt_settings[site_description]" id="llms_txt_site_description" class="large-text"
            rows="3"><?php echo esc_textarea($value); ?></textarea>
        <p class="description">
            <?php _e('Adicione uma descrição personalizada para o seu site no arquivo llms.txt. Esta descrição será exibida logo após o título do site.', 'llms-txt-generator'); ?>
        </p>
        <?php
    }
    ...
    public function field_include_posts_callback()
    {
        ...
        <p class="description">
            <?php _e('Se marcado, os posts publicados serão incluídos no arquivo llms.txt e o metabox será exibido na edição de posts.', 'llms-txt-generator'); ?>
        </p>
        <?php
    }
    ...
    public function field_include_pages_callback()
    {
        ...
        <p class="description">
            <?php _e('Se marcado, as páginas publicadas também serão incluídas no arquivo llms.txt e o metabox será exibido na edição de páginas.', 'llms-txt-generator'); ?>
        </p>
        <?php
    }
    ...
    public function field_custom_content_callback()
    {
        ...
        ?>
        <textarea name="llms_txt_settings[custom_content]" id="llms_txt_custom_content" class="large-text code"
        ...
        <div class="mt-2">
            <p class="text-sm">
                ...
                <a href="https://platform.openai.com/docs/guides/text-generation" target="_blank"
                    class="text-blue-600 hover:underline"><?php _e('Manual da API OpenAI', 'llms-txt-generator'); ?></a> |
                <a href="https://openrouter.ai/docs" target="_blank"
                    class="text-blue-600 hover:underline"><?php _e('Manual da API OpenRouter (DeepSeek)', 'llms-txt-generator'); ?></a>
            </p>
        </div>
        <?php
    }
    ...
    public function field_openai_api_key_callback()
    {
        ...
        <p class="description openai-api-fields" data-provider="openai">
            <?php _e('Insira sua chave da API OpenAI para habilitar a geração automática de descrições técnicas.', 'llms-txt-generator'); ?>
        </p>
        <p class="text-sm openai-api-fields" data-provider="openai"><a href="https://platform.openai.com/api-keys"
                target="_blank"
                class="text-blue-600 hover:underline"><?php _e('Obter chave da API OpenAI', 'llms-txt-generator'); ?></a></p>
        <?php
    }
    ...
    public function field_deepseek_api_key_callback()
    {
        ...
        <p class="description deepseek-api-fields" data-provider="deepseek">
            <?php _e('Insira sua chave da API OpenRouter para usar o modelo DeepSeek V3 (gratuito).', 'llms-txt-generator'); ?>
        </p>
        <p class="text-sm deepseek-api-fields" data-provider="deepseek"><a href="https://openrouter.ai/keys" target="_blank"
                class="text-blue-600 hover:underline"><?php _e('Obter chave gratuita da API OpenRouter', 'llms-txt-generator'); ?></a>
        </p>
        <?php
    }
    ...
    public function field_auto_generate_callback()
    {
        ...
        <p class="description">
            <?php _e('Se marcado, o plugin tentará gerar automaticamente descrições técnicas para posts e páginas que não possuem uma descrição personalizada.', 'llms-txt-generator'); ?>
        </p>
        <?php
    }
}
